...
| Command(s) | Description | Arguments |
| --- | --- | --- |
| `/domains`<br>`/dom` | Sends info about active domains in [Genshin Impact](https://genshin.mihoyo.com/en). | **Not needed, but accepted.**<br>`/domains` will send info about domains active today.<br>`/domains Mon` will send info about domains active on Monday.<br>`/domains -a` will send info about all domains (including artifact domains). |
