...
| `/echo` | Echoes any text that is input. Can also use replies to use others' text as input.<br>**This action can only be performed by admins of the bot.** | **Needed.**<br>`/echo _Some_ *text*` will output "\_Some\_ \*text\*".<br>`/echo -m _Some_ *text*` will output "*Some* **text**". |
